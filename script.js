"use strict";
      ...
      function applyClockSettings() {
        ...
        clock.style.fontWeight =
          settings.clockWeight === "bold" ? "700" : "400";
      ...
      function loadSettingsUI() {
        ...
        document.getElementById("clockWeightSetting").value =
          settings.clockWeight;
        document.getElementById("clockStyleSetting").value =
          settings.clockStyle;
        document.getElementById("timeFormatSetting").value =
          settings.timeFormat;
        document.getElementById("secondsSetting").checked =
      ...
      const onboarding = document.getElementById("onboarding"),
        ...
        onboardingProfileInput = document.getElementById(
      ...
      const GOOGLE_WEATHER_API_KEY = "AIzaSyAT2hx27KiqslNta_-5oX0xBCWP49WT6KM";
      async function loadWeather() {
        const el = document.getElementById("weatherContent");
        if (!GOOGLE_WEATHER_API_KEY) {
          el.innerHTML =
            '<div class="weather-status">Add your Google Weather API key to enable live weather.</div>';
          return;
        }
        ...
        navigator.geolocation.getCurrentPosition(
          async (pos) => {
            try {
              const url = `https://weather.googleapis.com/v1/currentConditions:lookup?key=${encodeURIComponent(GOOGLE_WEATHER_API_KEY)}&location.latitude=${pos.coords.latitude}&location.longitude=${pos.coords.longitude}&unitsSystem=METRIC`;
              ...
              const condition =
                ...
                "Current conditions";
              el.innerHTML = `<div><div class="weather-temp">${temperature ?? "—"}°</div></div><div class="weather-info"><div class="weather-condition">${escapeHtml(condition)}</div><div class="weather-location">Current location</div></div>`;
            } catch {
              el.innerHTML =
                '<div class="weather-status">Unable to load current weather.</div>';
            }
          },
          () =>
            (el.innerHTML =
      ...
      async function loadMedia() {
        const title = document.getElementById("mediaTitle"),
          artist = document.getElementById("mediaArtist");
        if (!globalThis.chrome?.tabs?.query) {
          title.textContent = "Nothing playing";
          artist.textContent = "Live browser media will appear here";
          return;
        }
        try {
          const tabs = await chrome.tabs.query({});
          const tab = tabs.find((x) => x.audible);
          if (!tab) {
            title.textContent = "Nothing playing";
            artist.textContent = "No audible browser tab";
            return;
          }
          title.textContent = tab.title || "Playing media";
          artist.textContent = "Active browser tab";
        } catch {
          title.textContent = "Nothing playing";
      ...
      function restoreFocusSession() {
        ...
        document
          ...
          .forEach((b) =>
            b.classList.toggle(
              "active",
              Number(b.dataset.minutes) === focusMinutes,
            ),
          );
        renderFocusTimer();
        renderHomeFocus();
